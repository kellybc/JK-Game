...
const VisualScene = ({ keyword }: { keyword: string }) => {
    // Specific high-quality images from Unsplash
    const IMAGES: Record<string, string> = {
        dragon: 'photo-1577493340887-b7bfff550145',
        orc: 'photo-1535581652167-3d6b98636db2',
        goblin: 'photo-1642436853767-f709da6598c1',
        skeleton: 'photo-1509557965875-b88c97052f0e',
        wolf: 'photo-1599488615731-7e5c2823ff28',
        bandit: 'photo-1542259681-d4cd7593c0ea',
        beast: 'photo-1552410260-0fd9b5779ab4',
        default: 'photo-1519074069444-1ba4fff66d16'
    };

    const getImageUrl = (k: string) => {
        const lowerK = k.toLowerCase();
        let id = IMAGES.default;
        if (lowerK.includes('dragon')) id = IMAGES.dragon;
        else if (lowerK.includes('orc') || lowerK.includes('troll')) id = IMAGES.orc;
        else if (lowerK.includes('goblin')) id = IMAGES.goblin;
        else if (lowerK.includes('skeleton') || lowerK.includes('undead')) id = IMAGES.skeleton;
        else if (lowerK.includes('wolf') || lowerK.includes('bear')) id = IMAGES.wolf;
        else if (lowerK.includes('bandit') || lowerK.includes('thief')) id = IMAGES.bandit;
        else if (lowerK.includes('beast') || lowerK.includes('monster')) id = IMAGES.beast;
    ...
    return (
        <div className="relative w-full h-full min-h-[400px] flex flex-col bg-slate-950 rounded-xl overflow-hidden border border-slate-700 shadow-2xl">
            ...
            <div className="relative z-10 flex flex-col h-full">
                ...
                {/* Player Footer */}
                <div className="p-4 bg-black/40 backdrop-blur-md border-t border-white/10 flex justify-between items-center mt-auto">
